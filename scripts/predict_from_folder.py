...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Predict A/B directly from a folder of paired *_c0/*_c1 images (c0->A, c1->B)."
    )
    ...
    ap.add_argument("--output", default="predictions_from_folder.jsonl")
    ap.add_argument(
        "--output-style",
        default="answer",
...
def collect_pairs(test_dir: Path, recursive: bool, strict: bool) -> tuple[list[PairSample], list[str]]:
    ...
    for key in ordered_keys:
        ...
        # Always force left=c0 (A), right=c1 (B)
        pair_id = f"test:{key}"
        samples.append(
            PairSample(
                source="test",
...
def main() -> None:
    ...
    probs, _ = predict_proba(bundle, pair_ds.X)
